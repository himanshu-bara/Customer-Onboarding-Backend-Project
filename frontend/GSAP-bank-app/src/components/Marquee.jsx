...
function Marquee() {
 let marqueeElem = Array.from({length:4}).map((ele,index)=>(
  <motion.h1 initial={{x:0}} animate={{x:"-100%"}} transition={{ease:"linear", repeat:Infinity, duration:10}} key={index} className='md:text-[20vw] text-[30vw] font-bold font-title md:-mb-6 pr-10'>We Are Oracle</motion.h1>
 ))
  return (
